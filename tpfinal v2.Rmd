...
```{r}
hist(dfwater$Chloramines, proba = T, col = grey(0.8), main = "")
curve(dnorm(x, mean=mean(dfwater$Chloramines), sd=sd(dfwater$Chloramines)), 
      add=TRUE, 
      col = "red")
```
...
for (k in 1:9) {
  v <- names(dfwater)[k]
  hist(dfwater[[v]], proba = T, col = grey(0.8), main = "", xlab=v)
  curve(dnorm(x, mean=mean(dfwater$Chloramines), sd=sd(dfwater$Chloramines)), 
              add=TRUE, 
              col = "red")
}
